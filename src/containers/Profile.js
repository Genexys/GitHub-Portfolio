...
class Profile extends Component {
  ...
  async componentDidMount() {
    try {
      ...
      if (profileJSON) {
        const repos = await fetch(profileJSON.repos_url + '?per_page=50');
  ...
  render() {
    ...
    const sortRepos = repos.sort((a, b) => {
      const one = new Date(a?.updated_at).getTime() / 1000;
      const two = new Date(b?.updated_at).getTime() / 1000;

      return one < two ? 1 : -1;
    });

    const reposList = sortRepos.map((repo) => ({
      label: repo.name,
